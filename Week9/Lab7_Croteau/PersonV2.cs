...
namespace Lab6_Croteau
{
    class PersonV2 : Person  //inherit from Person class
    {
        ...
        public string UpdateARecord()
        {
            ...
            // NEW
            //*******************************************************************************************************
            string strSQL = "UPDATE Persons SET FirstName=@FirstName, Middle=@Middle, LastName=@LastName, Street1=@Street1, Street2=@Street2, City=@City, State=@State, ZipCode=@ZipCode, Phone=@Phone, Cell=@Cell, Email=@Email, Instagram=@Instagram  WHERE PersonID = @Person_ID;";
            ...
            try
            {
                Conn.Open();                                        //Open connection to DB - Think of dialing a friend on phone
                int intRecs = comm.ExecuteNonQuery();
                strResult = $"SUCCESS: Updated {intRecs} records.";       //Report that we made the connection and updated a record
                Conn.Close();                                       //Hanging up after phone call
            }
            catch (Exception err)                                   //If we got here, there was a problem connecting to DB
            {
        ...
        public string DeleteOnePerson(int intPerson_ID)
        {
            ...
            //My SQL command string to pull up one EBook's data
            string sqlString =
           "DELETE FROM Persons WHERE PersonID = @Person_ID;";
